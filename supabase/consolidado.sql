...
-- ============================================================
-- 0006 — Endurecimiento: Security Advisor limpio
-- Mueve los helpers de RLS a un esquema `private` no expuesto por PostgREST,
-- retira la ejecución pública de funciones internas y agrega deny-all en
-- rate_limits. Resultado: Advisor sin hallazgos.
-- ============================================================
...
-- ============================================================
-- 0007 — Recepción sin acceso a estudios clínicos
-- ============================================================
...
-- ============================================================
-- 0008 — Módulo de Agenda de citas (sedes, horarios, citas)
-- ============================================================
...
create trigger trg_citas_touch before update on public.citas
  for each row execute function public.touch_updated_at();
...
from public.sedes s cross join (values (1),(2),(3),(4),(5),(6)) as d(dia)
...
from public.sedes s cross join (values (2),(4)) as d(dia)
...
-- ============================================================
-- 0009 — Historia Clínica Cardiológica (consultas)
-- ============================================================
do $$
begin
...
create policy consultas_update on public.consultas
  for update to authenticated
  using (private.puede('consultas', 'editar')) with check (private.puede('consultas', 'editar'));
...
-- ============================================================
-- 0010 — Evaluación Clínica Formal con firma (sellado inmutable)
-- ============================================================
do $$
begin
...
-- Sellado inmutable: una vez firmada, nadie (ni service_role) la altera ni borra.
...
create policy evaluaciones_update on public.evaluaciones
  for update to authenticated
  using (private.puede('evaluaciones', 'editar')) with check (private.puede('evaluaciones', 'editar'));
...
-- ============================================================
-- 0011 — Módulo Financiero + Recibos (pagos, gastos, categorías)
-- ============================================================
do $$
begin
...
create trigger trg_pagos_touch before update on public.pagos
  for each row execute function public.touch_updated_at();
...
create trigger trg_gastos_touch before update on public.gastos
  for each row execute function public.touch_updated_at();
...
create policy pagos_select on public.pagos for select to authenticated using (private.puede('pagos', 'ver'));
drop policy if exists pagos_insert on public.pagos;
create policy pagos_insert on public.pagos for insert to authenticated with check (private.puede('pagos', 'crear'));
...
create policy pagos_delete on public.pagos for delete to authenticated using (private.puede('pagos', 'borrar'));

drop policy if exists gastos_select on public.gastos;
create policy gastos_select on public.gastos for select to authenticated using (private.puede('finanzas', 'ver'));
drop policy if exists gastos_insert on public.gastos;
create policy gastos_insert on public.gastos for insert to authenticated with check (private.puede('finanzas', 'crear'));
...
create policy gastos_delete on public.gastos for delete to authenticated using (private.puede('finanzas', 'borrar'));

drop policy if exists categorias_select on public.categorias_gasto;
create policy categorias_select on public.categorias_gasto for select to authenticated using (private.puede('finanzas', 'ver'));
drop policy if exists categorias_write on public.categorias_gasto;
create policy categorias_write on public.categorias_gasto for all to authenticated
  using (private.puede('finanzas', 'editar')) with check (private.puede('finanzas', 'editar'));

insert into storage.buckets (id, name, public) values
  ('recibos', 'recibos', false), ('comprobantes', 'comprobantes', false)
on conflict (id) do nothing;

drop policy if exists recibos_obj_select on storage.objects;
create policy recibos_obj_select on storage.objects for select to authenticated
  using (bucket_id = 'recibos' and private.puede('pagos', 'ver'));
drop policy if exists recibos_obj_insert on storage.objects;
create policy recibos_obj_insert on storage.objects for insert to authenticated
  with check (bucket_id = 'recibos' and private.puede('pagos', 'crear'));
drop policy if exists recibos_obj_delete on storage.objects;
create policy recibos_obj_delete on storage.objects for delete to authenticated
  using (bucket_id = 'recibos' and private.puede('pagos', 'borrar'));

drop policy if exists comprobantes_obj_select on storage.objects;
create policy comprobantes_obj_select on storage.objects for select to authenticated
...
create policy comprobantes_obj_insert on storage.objects for insert to authenticated
...
create policy comprobantes_obj_delete on storage.objects for delete to authenticated
